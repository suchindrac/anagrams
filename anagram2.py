...
#
# Please execute this program using python 2.7
#
...
def anagram(input):
	...
	itr = ifilter(lambda x: len(x) <= len_input, dict_list)

	new_dict_list = []

	while 1:
		try:
			new_dict_list.append(itr.next())
		except:
			break
	...
	for i in xrange(1, len(red_words3) + 1):
